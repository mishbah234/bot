import os
import                 logger.error("Max retries reached - database connection failed")
                raise
...
def migrate_json_to_database():
    ...
    logger.info("Starting data migration from JSON files to database...")
        ...
        logger.    name = user.full_name or "Unknown"
...
def save_user        logger.error(f"Failed to save user {uid}: {e}")
        if conn:
            conn.rollback()
    finally:
        if conn:
